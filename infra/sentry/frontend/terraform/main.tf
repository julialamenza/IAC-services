...
data "template_file" "task_definition" {
  ...
  vars = {
    ...
    redis_host       = "sentry.f9udyh.ng.0001.euw3.cache.amazonaws.com"
...
data "template_file" "task_definition_worker" {
  template = "${file("${path.module}/task-definition-worker.json")}"
...
resource "aws_ecs_task_definition" "sentry-worker" {
  family                = "${var.service_name}_worker_td"
  container_definitions = "${data.template_file.task_definition_worker.rendered}"
}

resource "aws_ecs_service" "sentry-worker" {
  name            = "ecs-${var.service_name}-worker"
  cluster         = "${aws_ecs_cluster.main.id}"
  task_definition = "${aws_ecs_task_definition.sentry-worker.arn}"
  desired_count   = 1
  deployment_minimum_healthy_percent = 50
